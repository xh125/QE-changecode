...
FUNCTION efermig( et, nbnd, nks, nelec, wk, Degauss, Ngauss, is, isk )
  ...
  subroutine bisection_find_efermi(f, energy_lower_bound, energy_upper_bound, x, tol, Nmax, info)
    ...
    write(stdout,*) "Elw_local =",energy_lower_bound*rytoev," eV"
    write(stdout,*) "Eup_local =",energy_upper_bound*rytoev," eV"
      write(stdout,*) "f(Elw_local) =",f(Elw_local)
      write(stdout,*) "f(Eup_local) =",f(Eup_local)
